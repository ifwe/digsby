...
if getattr(sys, 'frozen', None) == "windows_exe":
    class Stderr(object):
        ...
        def write(self, text, alert=getattr(sys, '_MessageBox', lambda *a, **k: None),
                  fname=os.path.join(os.environ.get('LOCALAPPDATA', os.environ.get('APPDATA', '')), sys.executable + '.log')):
            if self._file is None and self._error is None:
                try:
                    try:
                        ...
                        try:
                            self._file = open(os.path.join(os.environ['temp'], os.path.basename(fname)), 'a')
                        except Exception:
                            raise details
                except Exception, details:
                    self._error = details
