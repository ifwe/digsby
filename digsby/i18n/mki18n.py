...
# ^^^^^^^^^^^^^^^
#
def makePO(applicationDirectoryPath,  applicationDomain=None, verbose=0, poDir=None, appFil=None) :
    ...
    if appFil is None:
        appFil = 'app.fil'
    if not os.path.exists(appFil):
        raise IOError(2,'No module file: %r' % appFil)

    if poDir is None:
        poDir = os.getcwd()

    messagesPot = os.path.join(poDir, 'messages.pot')
    ...
    cmd = 'xgettext -s -k_ --no-wrap --files-from="%s" --output=%s' % (appFil, messagesPot)
    if verbose: print cmd
    os.system(cmd)

    pofiles = [os.path.join(poDir, file) for file in os.listdir(poDir) if file.endswith('.po')]

    for pofile in pofiles:
        cmd = 'msgmerge -s --no-wrap "%s" "%s" > "%s.new"' % (pofile, messagesPot, pofile)
        if verbose: print cmd
        os.system(cmd)
    os.chdir(currentDir)
...
# ^^^^^^^^^^^^^^^
#
def makeMO(applicationDirectoryPath,targetDir='./locale',applicationDomain=None, verbose=0, forceEnglish=0,poDir=None) :
...
#    for langCode in languageDict.keys():
    ...
    if poDir is None:
        poDir = os.getcwd()
    os.chdir(poDir)
    pofiles = [file for file in os.listdir(poDir) if file.endswith('.po')]

    import re

    langCodeRE = re.compile('_'.join([applicationName, '(\w\w(?:_\w\w)?)\.po']))

    for pofile in pofiles:
        match = langCodeRE.match(pofile)
        if match is not None:
            langCode = match.group(1)
            mo_targetDir = "%s/%s/LC_MESSAGES" % (targetDir,langCode)
            if not os.path.exists(mo_targetDir):
                mkdir(mo_targetDir)
            cmd = 'msgfmt.exe --output-file="%s/%s.mo" "%s"' % (mo_targetDir,applicationName,pofile)
            if verbose: print cmd
            os.system(cmd)
    os.chdir(currentDir)
