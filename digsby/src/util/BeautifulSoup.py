...
from sgmllib import SGMLParser as _SGMLParser
class SGMLParser(_SGMLParser):
    def convert_charref(self, name):
        """Convert character reference, may be overridden."""

        #
        # Overridden from the standard library, to ignore characters between
        # 0 and 127, instead of between 0 and 255.
        #
        try:
            n = int(name)
        except ValueError:
...
class BeautifulStoneSoup(Tag, SGMLParser):
    ...
    def handle_data(self, data):
        if self.convertHTMLEntities and data:
            if data[0] == '&':
...
class BeautifulClosedSoup(BeautifulSoup):
    def isSelfClosingTag(self, name):
        return True
...
try:
    ...
    for obj in [PageElement, NavigableString, CData, ProcessingInstruction,
                Comment, Declaration, Tag, SoupStrainer, ResultSet,
                BeautifulStoneSoup, BeautifulSoup, BeautifulSOAP,
                UnicodeDammit]:
